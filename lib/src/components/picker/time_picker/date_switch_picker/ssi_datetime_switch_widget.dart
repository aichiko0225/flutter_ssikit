// ignore_for_file: unnecessary_null_comparison, prefer_conditional_assignment, no_logic_in_create_state, prefer_is_empty
...
class SsiDateTimeSwitchWidget extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _SsiDateTimeSwitchWidgetState(
      minDateTime, maxDateTime, initDateTime, minuteDivider);
}

class _SsiDateTimeSwitchWidgetState extends State<SsiDateTimeSwitchWidget> {
  ...
  _SsiDateTimeSwitchWidgetState(DateTime? minTime, DateTime? maxTime,
...
//    int currDate = initTime.difference(_baselineDate).inDays;
  ...
  Widget _renderDatePickerItemComponent(
    ...
    TextStyle? textStyle = SsiThemeConfigurator.instance.getConfig()?.pickerConfig?.itemTextStyle?.generateTextStyle();
    if ((ColumnType.Year == columnType && index == _calcSelectIndexList()[0]) ||
      ...
      textStyle = SsiThemeConfigurator.instance.getConfig()?.pickerConfig?.itemTextSelectedStyle?.generateTextStyle();
    }
