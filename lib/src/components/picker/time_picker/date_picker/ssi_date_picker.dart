...
import 'ssi_date_widget.dart';
import 'ssi_datetime_widget.dart';
import 'ssi_time_widget.dart';

///时间选择模式枚举
enum SsiDateTimePickerMode {
  /// Display DatePicker
  date,
...
class SsiDatePicker {
  ...
  /// pickerMode: [SsiDateTimePickerMode] display mode: date(DatePicker)、time(TimePicker)、datetime(DateTimePicker)
  ...
  static void showDatePicker(
    ...
    /// 时间选择组件显示的时间类型
    SsiDateTimePickerMode pickerMode = SsiDateTimePickerMode.date,
...
class _DatePickerRoute<T> extends PopupRoute<T> {
  ...
  _DatePickerRoute({
    ...
    this.themeData = SsiThemeConfigurator.instance.getConfig()?.pickerConfig?.merge(themeData);
  }
  ...
  final SsiDateTimePickerMode pickerMode;
  ...
  AnimationController createAnimationController() {
    ...
    _animationController = BottomSheet.createAnimationController(navigator?.overlay as TickerProviderStateMixin);
  ...
  Widget buildPage(
    ...
    double height = themeData?.pickerHeight ?? 0;
    if (pickerTitleConfig?.title != null || (pickerTitleConfig?.showTitle ?? false)) {
      height += themeData?.titleHeight ?? 0;
    }
  ...
  Widget build(BuildContext context) {
    ...
    switch (route.pickerMode) {
      ...
      case SsiDateTimePickerMode.datetime:
        pickerWidget = SsiDateTimeWidget(
          minDateTime: route.minDateTime,
          maxDateTime: route.maxDateTime,
          initDateTime: route.initialDateTime,
          dateFormat: route.dateFormat,
        ...
        builder: (BuildContext context, Widget? child) {
          return ClipRect(
            child: CustomSingleChildLayout(
              delegate: _BottomPickerLayout(route.animation?.value ?? 0, contentHeight: _pickerHeight),
...
class _BottomPickerLayout extends SingleChildLayoutDelegate {
  ...
  _BottomPickerLayout(this.progress, {this.contentHeight = 240});
