...
import 'package:flutter_ssikit/src/utils/ssi_multi_click_util.dart';
...
///
/// * [SsiBigMainButton], 大主色调按钮
/// * [SsiBigOutlineButton], 大边框按钮
/// * [SsiSmallMainButton], 小主色调按钮
/// * [SsiSmallOutlineButton], 小边框按钮
///
class SsiNormalButton extends StatelessWidget {
  /// 按钮是否可用 默认是true
  final bool isEnable;
  ...
  SsiNormalButton.outline({
    ...
    this.fontSize = _BFontSize,
    this.disableTextColor = _BTextColor,
    this.insertPadding = const EdgeInsets.only(
        left: _BHorizontalPadding, right: _BHorizontalPadding),
    this.textStyle,
    this.fontWeight = FontWeight.bold,
    ...
    this.borderRadius,
            ...
            disableLineColor: disableLineColor ?? const Color(0x00ffffff),
            lineColor: lineColor ?? const Color(0x00ffffff),
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        if (SsiMultiClickUtils.isMultiClick()) {
          return;
        }
  ...
  TextStyle _getTextStyle() {
    ...
    if (isEnable) {
      ...
      if (textColor == _BTextColor) {
        textColor = _BTextColor.withOpacity(0.7);
      }
    }
  ...
  BoxDecoration _getBoxDecoration(Color bgColor) {
    return BoxDecoration(
      color: bgColor,
      borderRadius:
          borderRadius ?? const BorderRadius.all(Radius.circular(_BRadius)),
    );
  }
}
...
class _OutlineBoxDecorationCreator {
  static BoxDecoration createOutlineBoxDecoration(
    ...
    return BoxDecoration(
        border: Border.all(
            color: _lineColor ?? const Color(0x00ffffff), width: borderWith),
