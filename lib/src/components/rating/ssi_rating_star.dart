...
typedef SsiRatingStarBuilder = Widget Function(RatingState state);
...
/// * 支持半颗星（仅支持展示，不支持选择）
class SsiRatingStar extends StatefulWidget {
  ...
  /// 单颗星星视图的自定义构造器
  final SsiRatingStarBuilder? starBuilder;
  ...
  @override
  _SsiRatingStarState createState() => _SsiRatingStarState();
}

class _SsiRatingStarState extends State<SsiRatingStar> {
  ...
  @override
  void didUpdateWidget(SsiRatingStar oldWidget) {
    currSelected = widget.selectedCount;
  ...
  Widget _buildRating(RatingState state) {
    switch (state) {
      case RatingState.select:
        return SsikitTools.getAssetSizeImage(SsiAsset.ICON_STAR, 16, 16);
      case RatingState.half:
        return SsikitTools.getAssetSizeImage(SsiAsset.ICON_STAR_HALF, 16, 16);
      case RatingState.unselect:
      default:
        return SsikitTools.getAssetSizeImage(SsiAsset.ICON_STAR, 16, 16, color: Color(0xFFF0F0F0));
    }
  }
}
