...
class SsiTextSelectFormItem extends StatefulWidget {
  final String? label;

  /// 录入项的唯一标识，主要用于录入类型页面框架中
  String type = SsiInputItemType.TEXT_SELECT_INPUT_TYPE;
  ...
  @override
  SsiTextSelectFormItemState createState() {
    return SsiTextSelectFormItemState();
  }
...
class SsiTextSelectFormItemState extends State<SsiTextSelectFormItem> {
  @override
  Widget build(BuildContext context) {
    return Container(
      color: Colors.white,
      padding: SsiFormUtil.computeItemEdgeInsets2(widget.prefixIconType, widget.isRequire),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          widget._isAutoLayout
  ...
  double _getTitleMaxWidth(BuildContext context, BoxConstraints constraints) {
    double contentRatio = SsiFormUtil.getAutoLayoutContentRatio(
        tipLabelHidden: widget.tipLabel == null || (widget.tipLabel?.isEmpty ?? false),
        layoutRatio: widget.layoutRatio);
    double maxWidth = min(
        constraints.maxWidth * contentRatio,
        _calculateTextWidth(context) +
            SsiFormUtil.rightArrowLeftPadding +
            SsiFormUtil.rightArrowSize);
    return maxWidth;
  }
    ...
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
      ...
      return Row(
        ...
        children: <Widget>[
          Flexible(
            child: Container(
              padding: SsiFormUtil.computeEdgeInsets2(widget.prefixIconType, widget.isRequire),
              child: Row(
                children: <Widget>[
                  SsiFormUtil.buildPrefixIcon(widget.prefixIconType, widget.isEdit, context, widget.onAddTap, widget.onRemoveTap),
  ...
  GestureDetector _buildRightWidget(BuildContext context) {
    return GestureDetector(
      onTap: () {
        ...
        SsiFormUtil.notifyTap(context, widget.onTap);
      },
      child: Container(
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: SsiFormUtil.getRightArrowIcon(),
            ),
          ],
        ),
      ),
          ...
          children: <Widget>[
            ...
            Container(
              constraints: BoxConstraints(
                  maxWidth: widget._isAutoLayout
                      ? SsiFormUtil.tipDescMaxWidth
                      : double.infinity),
              child: Text(
                widget.tipLabel ?? "",
                overflow: TextOverflow.ellipsis,
                strutStyle: StrutStyle(
                    forceStrutHeight: true, height: 1, leading: 0.5, fontSize: 14),
                style: SsiFormUtil.getTipsTextStyle(widget.themeData),
              ),
            ),
          ],
        ),
  ...
  Widget _buildTitleTextWidget() {
    return Text(
      ...
      strutStyle: _contentStructStyle,
      style: SsiFormUtil.getTitleTextStyle(widget.themeData, height: 1),
    );
  }
  ...
  double _calculateTextWidth(BuildContext context) {
    ...
    if (widget.value != null && (widget.value?.isNotEmpty ?? false)) {
      painter = TextPainter(
          ...
          text: TextSpan(
            text: widget.value,
            style: SsiFormUtil.getIsEditTextStyle(widget.themeData, widget.isEdit, height: 1),
          ));
    } else {
      painter = TextPainter(
          locale: Localizations.localeOf(context),
          ...
          text: TextSpan(
            text: widget.hint,
            style: SsiFormUtil.getHintTextStyle(widget.themeData, height: 1),
          ));
    }
    painter.layout();
  ...
  String getCalculateText() {
    String value = '请选择';
    if (!SsikitTools.isEmpty(widget.value ?? '')) {
      value = widget.value ?? '';
    } else if (!SsikitTools.isEmpty(widget.hint ?? '')) {
      value = widget.hint ?? '';
    }
    return value;
  }
