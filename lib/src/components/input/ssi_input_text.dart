...
/// 输入框输入变化的监听
typedef SsiInputTextChangeCallback = Function(String input);

/// 输入框提交的监听
typedef SsiInputTextSubmitCallback = Function(String input);

/// 输入完成点击键盘监听
typedef SsiInputTextEditingCompleteCallback = Function(String input);
...
///  支持设置输入最大字数限制
///  支持最大最小行数限制
class SsiInputText extends StatelessWidget {
  /// 搜索框输入内容改变时候的回调函数
  final SsiInputTextChangeCallback? onTextChange;

  /// 点击确定后的回调
  final SsiInputTextSubmitCallback? onSubmit;
              ...
              Text(
                ...
                style: TextStyle(
                  color: (currentLength == 0
                      ? Color(0xffcccccc)
                      : SsiThemeConfigurator.instance.getConfig()?.commonConfig?.colorTextSecondary),
                  fontSize: 16,
                ),
              ),
              Text(
