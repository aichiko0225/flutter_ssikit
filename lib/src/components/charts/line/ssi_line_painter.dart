...
class SsiLinePainter extends SsiBasePainter {
  ...
  /// x轴 刻度
  List<SsiDialItem> xDialValues;

  /// y轴左侧刻度显示，不传则没有
  List<SsiDialItem> yDialValues;
  ...
  bool isShowXText = true, isShowYText = true;
  ...
  @override
  bool shouldRepaint(SsiLinePainter oldDelegate) {
    return true;
  }
      ...
      for (var item in lines) {
        ...
        if (item.points != null && item.points.isNotEmpty) {
          ...
          if (item.isCurve) {
            ...
            _shadowPath.moveTo(pointArr[0].x.toDouble(), pointArr[0].y.toDouble());
            for (var i = 1; i < pointArr.length; i++) {
              _path.lineTo(pointArr[i].x.toDouble(), pointArr[i].y.toDouble());
              _shadowPath.lineTo(pointArr[i].x.toDouble(), pointArr[i].y.toDouble());
  ...
  ///x轴刻度 & 辅助线
  void _drawXRuler(Canvas canvas, Paint paint, List<SsiPointData> points) {
    if (xDialValues != null && xDialValues.isNotEmpty) {
      // 获取刻度长度
      ...
      if (element.points != null) {
        ...
        element.points?.forEach((pointElement) {
          ...
          var pointPaintBg = Paint()
            ...
            ..color = color
            ..style = PaintingStyle.fill;
          canvas.drawCircle(Offset(pointElement.x.toDouble(), pointElement.y.toDouble()),
  ...
  _drawSelectPointWithIndex(Canvas canvas, Paint paint) {
    ...
    var xPath = Path()
      ..moveTo(x.toDouble(), _startY)
      ..lineTo(x.toDouble(), _endY - basePadding);
    var y = _linePointPositions[lineSelectIndex][pointSelectIndex].y;
    var yPath = Path()
      ..moveTo(_startX, y.toDouble())
      ..lineTo(_endX, y.toDouble());
  ...
  void _drawPointDisplayText(Canvas canvas) {
    if (_linePointPositions != null && _linePointPositions.isNotEmpty) {
      for (int lineIndex = 0;
          ...
          lineIndex++) {
        SsiPointsLine item = lines[lineIndex];
