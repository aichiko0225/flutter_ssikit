...
import 'package:flutter_ssikit/src/components/charts/line/ssi_line_painter.dart';
import 'package:flutter_ssikit/src/components/charts/line/ssi_line_y_painter.dart';
...
/// 支持数据过多时左右滑动查看数据
/// 支持 X、Y 轴自定义展示文本内容
class SsiBrokenLine extends StatefulWidget {
  /// 表格宽高
  final Size size;

  /// 绘制线条的参数内容
  final List<SsiPointsLine> lines;
  ...
  /// y轴左侧刻度显示，不传则没有
  final List<SsiDialItem> yDialValues;

  /// x 轴刻度
  final List<SsiDialItem> xDialValues;
  ...
  @override
  State<StatefulWidget> createState() => SsiBrokenLineState();
}

class SsiBrokenLineState extends State<SsiBrokenLine> {
  int pointSelectIndex = -1;
  int lineSelectIndex = -1;

  @override
  Widget build(BuildContext context) {
    var _lineWithXPainter = SsiLinePainter(
      widget.lines,
