...
class DialogEntryPage extends StatelessWidget {
  ...
  const DialogEntryPage(this._title,{Key? key }) : super(key: key);
        ...
        body: ListView(
          children: <Widget>[
            ...
            ListItem(
              ...
              onPressed: () {
                _showSsiOneVerticalButtonDialogDialog(context);
              },
            ),
            ListItem(
              ...
              isSupportTheme: true,
              onPressed: () {
                _showSsiTwoVerticalButtonDialogDialog(context);
              },
            ),
            ListItem(
              title: "纯文本弹框",
  ...
  void _showMiddleMultiSelectDialog(BuildContext context) {
    ...
    showDialog(
        context: context,
        builder: (_) => SsiMultiSelectDialog(
            title: "请您评价该条线索",
            isClose: true,
            conditions: data,
  ...
  void _showRichTextDialog(BuildContext context) {
    SsiDialogManager.showConfirmDialog(context,
        cancel: "取消",
        confirm: "确定",
        title: "标题",
  ...
  void _showMiddleInputDialog(BuildContext context) {
    SsiMiddleInputDialog(
        ...
        onConfirm: (value) {
          SsiToast.show(value, context);
        },
        onCancel: () {
          SsiToast.show("取消", context);
          Navigator.pop(context);
        }).show(context);
  }
  ...
  void _showStyle5_1Dialog(BuildContext context) {
    SsiDialogManager.showMoreButtonDialog(context,
        actions: [
          '选项一',
          '选项二',
          '选项三',
  ...
  void _showStyle71Dialog(BuildContext context) {
    SsiDialogManager.showConfirmDialog(context,
        showIcon: true,
        iconWidget: SsikitTools.getAssetImage("icons/icon_warning.png"),
  ...
  ///底部有输入框弹框
  void _showSsiLoadingDialog(BuildContext context) {
    SsiLoadingDialog.show(context);

    Future.delayed(Duration(seconds: 10)).then((_) {
      SsiLoadingDialog.dismiss(context);
      SsiToast.show('定时取消', context);
    });
  }

  void _showSsiShareDialog3(BuildContext context) {
    SsiShareDialog ssiShareDialog = SsiShareDialog(
      context: context,
      shareChannels: const [
        SsiShareItemConstants.SHARE_WEIXIN,
        SsiShareItemConstants.SHARE_LINK,
        SsiShareItemConstants.SHARE_CUSTOM
      ],
      titleText: "测试标题",
      descText: "测试辅助信息测试辅助信息测试辅助信息测试辅助信息测试辅助信息",
      clickCallBack: (int channel, int index) {
        SsiToast.show("channel: $channel, index: $index", context);
      },
      getCustomChannelWidget: (int index) {
        if (index == 2) {
          return SsikitTools.getAssetImage("images/icon_custom_share.png");
        } else {
          return null;
        }
      },
  ...
  void _showSsiShareDialog5(BuildContext context) {
    SsiShareDialog ssiShareDialog = SsiShareDialog(
      context: context,
      shareChannels: const [
        SsiShareItemConstants.SHARE_WEIXIN,
  ...
  void _showSsiTwoVerticalButtonDialogDialog(BuildContext context) {
    SsiEnhanceOperationDialog ssiShareDialog = SsiEnhanceOperationDialog(
      context: context,
      iconType: SsiDialogConstants.ICON_ALERT,
      titleText: "强提示文案",
  ...
  void _showSsiOneVerticalButtonDialogDialog(BuildContext context) {
    SsiEnhanceOperationDialog ssiShareDialog = SsiEnhanceOperationDialog(
      iconType: SsiDialogConstants.ICON_WARNING,
      context: context,
      titleText: "强提示文案",
      descText: "这里是文案这里是文案这里是文案这里是文案这里是文案这里是文案这里是文案这里是文案",
      mainButtonText: "我知道了",
  ...
  void _showStyle82Dialog(BuildContext context) {
    showDialog(
        context: context,
        builder: (_) => SsiScrollableTextDialog(
            title: "纯文本标题",
            contentText:
            ...
            onSubmitClick: () {
              SsiToast.show("点击了纯文本弹框", context);
            }));
  }
  ...
  void _showStyle84Dialog(BuildContext context) {
    showDialog(
        context: context,
        builder: (_) => const SsiScrollableTextDialog(
              title: "纯文本标题纯文",
              contentText: "纯本文呢表纯本文呢表纯本文呢表纯本文呢表纯本文呢呢表纯本文呢表纯本文呢"
                  "表纯本文呢表纯本文呢呢表纯本文呢表纯本文呢表纯本文呢表纯本文呢呢表"
                  "纯本文呢表纯本文呢表纯本文呢表纯本文呢呢表纯本文呢表纯本文呢表纯本"
